...
const getStatusColor = (status: string) => {
  ...
  if (status === 'inprogress') return { bg: 'rgba(87,157,255,0.15)', color: 'var(--accent-blue)' };
  if (status === 'done') return { bg: 'rgba(75,206,151,0.15)', color: 'var(--accent-green)' };
  ...
  if (isLoading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'var(--text-secondary)' }}>
        <div className="auth-spinner" style={{ marginRight: 12 }} /> Loading tasks...
      </div>
    );
  }
  ...
  return (
    <div className="backlog-page">
      <div className="backlog-header">
        <h2>List View</h2>
        <p>A comprehensive table view of all tasks across the project.</p>
      </div>

      <div className="backlog-table-container">
        {cards.length === 0 ? (
          <div className="backlog-empty">No tasks found in the project.</div>
        ) : (
